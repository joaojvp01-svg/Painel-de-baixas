...
import {
  ...
  Wallet, LayoutGrid, Filter, Clock, AlertTriangle, Download, Share2, Copy, Check, ExternalLink
} from "lucide-react";
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
        ...
        <KpiCard icon={DollarSign} label="Valor entregue" accent={COLORS.blue} value={fmtMoneyCompact(o.valorEntregue)} sub="Valor Venda das entregues" />
      </div>
      ...
      {pendVariacao !== null && (
        <div style={{
          ...
          {" "}({fmtInt(currentStats.pendente)} notas · {fmtMoney(currentStats.valorPendente)} em risco)
        </div>
      )}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
        ...
        <KpiCard icon={DollarSign} label="Valor pendente" accent={COLORS.navy} value={fmtMoneyCompact(stats.valorPendente)} sub={isCurrent ? "mês atual" : selectedMonth} />
      </div>
      ...
      <Panel title="Scorecard comparativo" subtitle="Volume, pendências, SLA e aging médio por transportadora — uso interno">
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12.5, minWidth: 620 }}>
            <thead><tr>
              {["Transportadora", "Total NF", "Pendente", "% Pend.", "SLA no prazo", "Fora prazo", "Aging médio", "Valor em risco"].map((h, i) => (
            ...
            <tbody>
              {summary.carrierScorecard.map((c) => (
                <tr key={c.transportadora}>
                  ...
                  <td style={{ padding: "7px 10px", borderBottom: `1px solid ${COLORS.border}`, textAlign: "center", fontFamily: "ui-monospace, monospace", fontWeight: 600 }}>{c.valorPendente ? fmtMoneyCompact(c.valorPendente) : "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
/*  Tab 5 — Financeiro                                                  */
/* ------------------------------------------------------------------ */
function Financeiro({ summary }) {
  const o = summary.overall;
  const monthlyMoneyData = useMemo(() => summary.monthsAsc.map((m) => {
    const s = summary.monthlyStats[m];
    return { mes: m, "Valor entregue": Math.round(s.valorEntregue), "Valor pendente": Math.round(s.valorPendente) };
  }), [summary]);

  const carrierMoneyData = useMemo(() => [...summary.carrierScorecard]
    .filter((c) => c.valorPendente > 0)
    .sort((a, b) => b.valorPendente - a.valorPendente)
    .map((c) => ({ transportadora: c.transportadora, "Valor pendente": Math.round(c.valorPendente) })), [summary]);

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
        <KpiCard icon={DollarSign} label="Valor total movimentado" accent={COLORS.navy} value={fmtMoneyCompact(o.valorTotal)} sub="Soma do Valor Venda de todas as notas" />
        <KpiCard icon={PackageCheck} label="Valor entregue" accent={COLORS.green} value={fmtMoneyCompact(o.valorEntregue)} sub={fmtPct(o.valorTotal ? (o.valorEntregue / o.valorTotal) * 100 : 0)} />
        <KpiCard icon={PackageX} label="Valor em risco (pendente)" accent={COLORS.orange} value={fmtMoneyCompact(o.valorPendente)} sub={fmtPct(o.valorTotal ? (o.valorPendente / o.valorTotal) * 100 : 0)} />
      </div>

      <Panel title="Valor entregue vs. pendente por mês" subtitle="Exposição financeira ao longo do tempo">
        <ResponsiveContainer width="100%" height={280}>
          <BarChart data={monthlyMoneyData} margin={{ top: 4, right: 12, left: 10, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke={COLORS.border} vertical={false} />
            <XAxis dataKey="mes" tick={{ fill: COLORS.textFaint, fontSize: 11 }} axisLine={{ stroke: COLORS.border }} tickLine={false} />
            <YAxis tick={{ fill: COLORS.textFaint, fontSize: 11 }} axisLine={false} tickLine={false} tickFormatter={(v) => fmtMoneyCompact(v)} width={70} />
            <Tooltip content={<CustomTooltip money />} cursor={{ fill: "rgba(255,105,1,0.05)" }} />
            <Legend wrapperStyle={{ fontSize: 12, color: COLORS.textDim }} />
            <Bar dataKey="Valor entregue" fill={COLORS.green} radius={[4, 4, 0, 0]} />
            <Bar dataKey="Valor pendente" fill={COLORS.orange} radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </Panel>

      <Panel title="Valor pendente por transportadora" subtitle="Onde está concentrado o risco financeiro em aberto">
        <ResponsiveContainer width="100%" height={Math.max(180, carrierMoneyData.length * 34)}>
          <BarChart data={carrierMoneyData} layout="vertical" margin={{ top: 4, right: 40, left: 10, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke={COLORS.border} horizontal={false} />
            <XAxis type="number" tick={{ fill: COLORS.textFaint, fontSize: 11 }} axisLine={false} tickLine={false} tickFormatter={(v) => fmtMoneyCompact(v)} />
            <YAxis type="category" dataKey="transportadora" tick={{ fill: COLORS.textDim, fontSize: 12 }} axisLine={false} tickLine={false} width={190} />
            <Tooltip content={<CustomTooltip money />} cursor={{ fill: "rgba(255,105,1,0.05)" }} />
            <Bar dataKey="Valor pendente" fill={COLORS.orange} radius={[0, 4, 4, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </Panel>
    </div>
...
function buildStaticReportHTML(summary) {
  ...
  const kpisHTML = `
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:12px;margin-bottom:20px;">
      ...
      ${kpiCard("Valor entregue", fmtMoneyCompact(o.valorEntregue), "Valor Venda das entregues", COLORS.blue)}
    </div>`;
  ...
  const alertHTML = pendVariacao !== null ? `
    <div style="display:flex;align-items:center;gap:8px;font-size:12.5px;padding:10px 14px;border-radius:8px;margin-bottom:20px;
      ...
      em relação a ${summary.monthsAsc[idx - 1]} (${fmtInt(currentStats.pendente)} notas · ${fmtMoney(currentStats.valorPendente)} em risco)
    </div>` : "";
  ...
  const moneyByCarrier = [...summary.carrierScorecard].filter((c) => c.valorPendente > 0).sort((a, b) => b.valorPendente - a.valorPendente);
  const maxCarrierMoney = Math.max(1, ...moneyByCarrier.map((c) => c.valorPendente));
  const moneyRows = moneyByCarrier.map((c) => {
    const pctBar = (c.valorPendente / maxCarrierMoney) * 100;
    return `
      <tr>
        <td style="padding:7px 10px;border-bottom:1px solid ${COLORS.border};white-space:nowrap;">${c.transportadora}</td>
        <td style="padding:7px 10px;border-bottom:1px solid ${COLORS.border};">
          <div style="display:flex;align-items:center;gap:8px;">
            ...
            <span style="font-family:ui-monospace,monospace;font-weight:700;color:${COLORS.text};min-width:80px;text-align:right;">${fmtMoneyCompact(c.valorPendente)}</span>
          </div>
        </td>
      </tr>`;
  }).join("");
...
<body style="margin:0;background:${COLORS.bg};color:${COLORS.text};font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
  <div style="max-width:1100px;margin:0 auto;padding:24px 20px 60px;">
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:14px;margin-bottom:22px;padding-bottom:18px;border-bottom:1px solid ${COLORS.border};">
      <div style="display:flex;align-items:center;gap:12px;">
        ...
        <div>
          ...
          <div style="font-size:12px;color:${COLORS.textFaint};">${fmtInt(summary.total)} notas fiscais · ${fmtMoneyCompact(summary.overall.valorTotal)} movimentados</div>
        </div>
      </div>
    ...
    <div style="background:#fff;border:1px solid ${COLORS.border};border-radius:10px;padding:16px;box-shadow:0 1px 2px rgba(16,24,40,0.04);overflow-x:auto;">
      <table style="width:100%;border-collapse:collapse;font-size:12.5px;min-width:620px;">
        <thead><tr>
          ...
          <th style="text-align:center;padding:8px 10px;color:${COLORS.textDim};font-weight:600;border-bottom:1px solid ${COLORS.border};">Valor em risco</th>
        </tr></thead>
    ...
    <h2 style="font-size:15px;margin:28px 0 12px;">Financeiro — valor pendente por transportadora</h2>
    <div style="background:#fff;border:1px solid ${COLORS.border};border-radius:10px;padding:16px;box-shadow:0 1px 2px rgba(16,24,40,0.04);">
      <table style="width:100%;border-collapse:collapse;font-size:12.5px;">
    ...
    <div style="text-align:center;font-size:11px;color:${COLORS.textFaint};margin-top:28px;">
      Uso interno — inclui comparativo entre transportadoras e dados financeiros. Não recomendado para compartilhamento externo.
    </div>
  </div>
</body>
</html>`;
    ...
    <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: COLORS.textFaint }}>
              {summary ? `${fmtInt(summary.total)} notas fiscais · ${fmtMoneyCompact(summary.overall.valorTotal)} movimentados` : "Aguardando dados"} · visão interna
            </div>
          </div>
        </div>
      ...
      {!loading && summary && (
        <>
          ...
          {activeTab === "financeiro" && <Financeiro summary={summary} />}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: COLORS.textFaint, justifyContent: "center", paddingTop: 20 }}>
            <ChevronRight size={12} /> Uso interno — inclui comparativo entre transportadoras e dados financeiros. Não recomendado para compartilhamento externo.
          </div>
        </>
      )}
